...
def process_file(filename):
    ...
    result = analyze_log(log, pipeline_name=filename)
...
def analyze_all_logs():
    ...
    summary_path = os.path.join(os.path.dirname(__file__), "summary_report.json")
...
def run_jenkins_agent():
    print("\n🚀 Jenkins Pipeline Agent Starting...")
    print("   Make sure mock_jenkins.py is running in another terminal")
    print("   Command: py -3.10 mock_jenkins.py")
    print("\n" + "="*50)

    jenkins_path = os.path.join(os.path.dirname(__file__), "jenkins_agent.py")

    if not os.path.exists(jenkins_path):
        print("❌ jenkins_agent.py not found in backend/ folder")
        return

    try:
        subprocess.run([sys.executable, jenkins_path])
    except KeyboardInterrupt:
        print("\n⚠️  Jenkins agent stopped by user")
    except Exception as e:
        print(f"❌ Error running Jenkins agent: {e}")
        print("   Run manually: py -3.10 jenkins_agent.py")
...
def main():
    while True:
        ...
        print("║  4. Run Jenkins Pipeline Agent        ║")
        print("║  5. Exit                              ║")
        print("╚══════════════════════════════════════╝")
