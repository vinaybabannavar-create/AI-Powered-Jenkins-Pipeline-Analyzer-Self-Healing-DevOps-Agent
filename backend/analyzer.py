...
PATTERNS = {
    ...
    "Code Defect": [
        ...
        r"uncaught exception", r"exit code 1",
        r"process crashed", r"build process"
    ],
    "Configuration Error": [
...
def analyze_log(log, pipeline_name="unknown"):
    ...
    if category and confidence in ["High", "Medium"]:
        ...
        return {
            ...
            "category": "Build Error"    if category in ["Dependency Issue", "Configuration Error"] else
                        ...
                        "Infrastructure" if category in ["Infrastructure Issue", "Timeout"] else
                        "Execution",
    ...
    result = llm_classify(log, pipeline_name)
    result["source"] = "llm"
    return result
...
def take_action(result):
    ...
    if action_type == "Flaky Test":
        ...
        summary = (
            ...
            f"Pipeline : {result.get('pipeline', 'unknown')}\n"
            f"Reason   : {result['reason']}\n"
            f"Fix      : {result['fix']}\n"
            f"Confidence: {result['confidence']}\n"
            f"Source   : {result.get('source', 'unknown')}\n"
        )
        with open("jira_issues.txt", "a", encoding="utf-8") as f:
            f.write(summary + "="*40 + "\n")
        ...
        with open("config_suggestions.txt", "a", encoding="utf-8") as f:
            f.write(suggestion + "="*40 + "\n")
